...
//--spring.batch.job.names=youtubeJob
...
public class YoutubeApiBatchConfig {
    ...
    @Bean
    public Job youtubeJob(Step step1) throws Exception {
    ...
    @JobScope
    @Bean
    public Step step1(ItemReader videoReader, ItemProcessor videoProcessor, ItemWriter videoWriter) throws Exception {
        log.info("---------------- Step1 시작 ----------------");
...
//                chunk<O> : ItemReader 로 부터 받아온 데이터를 ItemProcessor 로 가공한 후 ,  ItemWriter 에 전달되는 타입
                .<YoutubeVideoInfo, List<Video>>chunk(1100)
                .reader(videoReader)
